            ...
            try:
                ...
                for row in row_range:
                    ...
                    if is_app_estimate:
                        ...
                        item_name = ws.cell(row=row, column=2).value  # Column B (Item Name)
                        quantity_cell = ws.cell(row=row, column=3).value  # Column C (Qty)
                        unit_cell = ws.cell(row=row, column=4).value  # Column D (Unit)
                        rate_cell = ws.cell(row=row, column=5).value  # Column E (Rate)
                        total_price_cell = ws.cell(row=row, column=6).value  # Column F (Total)
                    else:
                        # For regular Excel files (original format)
